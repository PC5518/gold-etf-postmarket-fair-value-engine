# ============================================
#  ANSCom Terminal v4.1 - Stability Patch
#  Fixes: ArtistList error, Layout Compactness
# ============================================
...
def add_branding_header(fig):
    ...
    # Reduced size and moved to corner to save space
...
def get_usdinr_at_1530_yf(india_date_str):
    ...
    try:
        ...
        # Added auto_adjust=True to silence warning and get correct price
...
def run_terminal(gold_symbol="XAUUSD"):
    ...
    # Chart Titles
    ax_chart_etf.set_title(f"Synthetic ETF (Live)", color=COLOR_TEXT_W, fontsize=10, loc='left', pad=10)
    ax_chart_gold.set_title(f"{gold_symbol} Spot (Live)", color=COLOR_TEXT_W, fontsize=10, loc='left', pad=10)
    ...
    try:
        while True:
            ...
            if time.time() - last_ui_update > 0.1:
                ...
                # 3. Update Charts (FIXED CLEAR METHOD)
                # Instead of ax.collections.clear(), we remove items safely
                ...
                # 4. Data Factory (Clean List Look)
                ax_data_fact.clear()
                ...
                def print_row(label, val_fmt, key, is_header=False):
                    ...
                    if key:
                        try:
                            numeric_val = float(str(val_fmt).strip('%').strip('+'))
